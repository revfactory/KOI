...
> **본 문서의 위상**
>
> 이 문서는 6주 교육 콘텐츠에 필요한 **외부 자료, 공식 문서, 사례 OSS 저장소, AI 도구, 라이선스 케이스, 학습 리소스, 한국 OSS 후보**를 단일 출처로 모은 자료 풀이다. 강의자료와 운영 문서에서 외부 자료를 인용할 때 이 표의 제목·발행처·발행일·URL을 기준으로 맞춘다.
>
> 충돌이 발생하면 `01_master_plan.md`의 회차별 핵심 메시지가 우선이며, 본 문서는 그 메시지에 맞는 외부 자료를 제공한다.
...
아래 표는 기존 인용 번호 [1]~[11]에 대응하는 1차 출처를 2026-05-04 시점에 확인한 결과다. **검증 컬럼**: ✓ = 접근 확인 + 발행처 일치 / △ = 접근 가능하나 URL 형식 변동 가능 / × = 접근 불가, archive.org 대안 사용.

| # | 기존 표기 | 정확한 제목 | 발행처 | 발행일 | URL | 검증 | archive.org 대안 |
|---|----------|--------|------|------|-----|----|------------|
...
| Choose a License | Choose an open source license | GitHub | live | https://choosealicense.com/ | 3회차 라이선스 선택 도구 |
...
회차 강의자료 본문에서 인용은 다음 형식 중 하나를 사용한다.
...
**금지**: "GitHub 공식 문서에 따르면" 같이 URL 없이 막연한 인용.
...
>
> **이 회차 자료 풀의 역할**: GitHub Trending·Octoverse·인기 OSS 분석 5축(README/이슈/PR/릴리즈/라이선스) 실습에 쓸 실제 저장소를 공급한다. "별 수가 많다고 좋은 게 아니다"라는 과정 핵심 메시지를 살리려면 README 품질·이슈 관리·최근 커밋·기여 가이드·라이선스가 갖춰진 곳을 우선.
...
§4 AI 도구 카탈로그의 비교표를 함께 사용하고, 학생용 무료 옵션 컬럼을 강조한다.
...
>
> **공통 결론**: *"품질과 자율성"은 Claude Code, "IDE 일체형 경험"은 Cursor, "비용 효율"은 GitHub Copilot Free, "BYOM·OSS"는 Continue/Aider. 학생은 무료 티어부터 시작.*
...
- **huggingface/transformers**: 한국 사용자가 많고 한국어 docs PR을 환영한다. https://github.com/huggingface/transformers
- **freeCodeCamp 한국어 번역**: 한국어 contribution이 직접 가능하다. https://github.com/freeCodeCamp/freeCodeCamp
- **MDN Web Docs 한국어**: 번역 PR 진입이 친절하다. https://github.com/mdn/translated-content

### 7.4 한국 OSS 검색 시 깨진 링크/주의 사항

- 일부 회사(특히 게임사·중소기업)는 GitHub에 공개 저장소를 두었다가 비공개 전환하는 경우가 있으니, 회차 자료 인용 직전 1회 더 접근 확인 권장.
- NAVER 관련 organization은 `NAVER`, `naver-ai`, `navervision`, `clovaai`, `NaverCloudPlatform` 등으로 분리되어 있으니 저장소 URL을 정확히 확인한다.
